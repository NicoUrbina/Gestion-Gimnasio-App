...
export default function PaymentFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validaciones
    if (formData.payment_type === 'membership' && !formData.membership) {
      alert('Selecciona una membresía para pago de membresía');
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-400 mt-1">Sube tu comprobante y será revisado</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-300 mb-2">
              Tipo de Pago <span className="text-orange-500">*</span>
            </label>
            <select
            ...
            >
              <option value="membership">Pago de Membresía</option>
              <option value="product">Compra de Producto</option>
              <option value="service">Servicio Adicional</option>
              <option value="other">Otro</option>
            </select>
          </div>

          {/* Membresía (solo si payment_type === 'membership') */}
          {formData.payment_type === 'membership' && (
            <div className="md:col-span-2">
              ...
              <select
                ...
                required={formData.payment_type === 'membership'}
                className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
              >
          ...
          <div className="md:col-span-2">
            ...
            {!receiptPreview ? (
              <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-zinc-700 rounded-xl cursor-pointer hover:border-orange-500 hover:bg-zinc-800/50 transition-colors">
              ...
              <div className="relative">
                <img
                  src={receiptPreview}
                  alt="Comprobante"
                  className="w-full h-60 object-contain bg-zinc-800 rounded-xl border border-zinc-700"
                />
                <button
                  type="button"
        ...
        <div className="mt-4 p-4 bg-orange-500/10 border border-orange-500/20 rounded-xl">
          <p className="text-sm text-orange-200">
            <strong className="text-orange-400">Nota:</strong> Tu pago será revisado por el personal del gimnasio.
